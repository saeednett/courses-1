...
$(document).ready(function () {
    ...
    let view_older = $("#viewHolder");
    let blocker = $(".blocker");
    let error_holder = $(".error_holder");
    let custom_active = $(".custom-active");
    ...
    function removeView() {
        var length = view_older.children().length;
        return new Promise(function (resolve, reject){

            // $("#viewHolder").children().remove();
            for (let i = 0; i < view_older.children().length; i++){
                view_older.children().eq(i).fadeOut(800, function () {

                    if ( (i + 1) === length ){
                        resolve("done");
                    }

                    view_older.children().eq(i).remove();
    ...
    function createNullView(){
        return new Promise(function (resolve, reject) {
            view_older.append("<div class='col-12 mt-4'> <h2 class='text-right text-danger'>لاتوجد دورات حسب التصنيف المطلوب</h2> </div>");
            resolve("done");
        })
    }
    ...
    filter_tabs.click(function () {
        ...
        if ( $(this).hasClass('custom-active') ){
            ...
            custom_active.removeClass('custom-active');
