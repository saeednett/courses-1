...
@section('style-file')
    <link rel="stylesheet" href="{{ asset('css/administrator/show-centers.css') }}">
@endsection
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-bordered table-hover dataTables-example">
                            ...
                            <tr>
                                ...
                                <th class="text-center">المسؤلين</th>
                            ...
                            @if(count($centers) > 0)
                                @foreach($centers as $center)
                                    <tr class="gradeX">
                                        ...
                                        <td>{{ count($center->trainer) }}</td>
                                        <td>{{ count($center->admin) }}</td>
                                        <td class="ltr">{{ date("Y-m-d", strtotime($center->created_at)) }}</td>

                                        @if($center->user->status == 1)
                                            <td class="text-success">مفعلة</td>
                                        @else
                                            <td class="text-danger">غير مفعلة</td>
                                        @endif
                                    </tr>
                                @endforeach
                            @else
