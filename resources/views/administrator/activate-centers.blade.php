...
@section('title', 'تفعيل الجهات')

@section('style-file')
    <link rel="stylesheet" href="{{ asset('css/administrator/activate-centers.css') }}">
@endsection
            ...
            <div class="modal-dialog">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body text-center">
                        <p class="text-danger">عند إلغاء تفعيل الجهة سوف يتم منع تسجيل الدخول إلى أن يتم إعادة التفعيل</p>
                    </div>
        ...
        <div class="col-lg-12 animatedParent animateOnce z-index-50">
            <div class="panel panel-default animated fadeInUp">
                <div class="panel-heading clearfix">
                    <h3 class="panel-title">معلومات الجهات المسجلة في النظام</h3>
                ...
                <div class="panel-body">
                            ...
                            @if(count($centers) > 0)
                                @foreach($centers as $center)
                                    <tr class="gradeX">
                                        ...
                                        <td class="ltr pt-18">{{ $center->user->phone }}</td>
                                        <td class="pt-18">{{ $center->user->email }}</td>
                                        @if($center->type == 1)
                                            <td class="pt-18">ربحية</td>
                                        @else
                                            <td class="pt-18">غير ربحية</td>
                                        @endif
                                        <td class="pt-18">{{ count($center->course) }}</td>
                                        <td class="ltr pt-18">{{ date("Y-m-d", strtotime($center->created_at)) }}</td>
                                        <td>

                                            @if($center->user->status == 1)
                            ...
                            @else
                                <tr class="gradeX">
                                    <td class="text-danger" colspan="8">
                                        <h3 class="mt-15">لايوجد جهات مسجلة في النظام</h3>
                                    </td>
                                </tr>
                            @endif
                            </tbody>
                    ...
                    @if(count($centers) > 0)
                        <form method="post" action="{{ route('administrator.centers.activation.deactivation.confirm') }}"
                              id="activation-form">
                            {{ csrf_field() }}
...
@section('script-file')
    ...
    <script src="{{ asset('js/administrator/activate-centers.js') }}"></script>
@endsection